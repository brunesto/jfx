...
public class DatePickerOnFocusLostTest {
    ...
    // 4. Verify that in case of typo, the datePicker kept the focus, onChangeListener was not called and  the property value remained unchanged.
    public void testdatePickerCommit(boolean typo) throws Exception {
        ...
        // 4 check that the focus ends up on button only if there is no typo (fixed by patch)
        Assert.assertEquals(typo, datePicker.isFocused());
        Assert.assertEquals(!typo, button.isFocused());

        // 5 check that in case of typo, the value has not changed (this was already working as expected before the patch )
        Assert.assertEquals(typo, datePicker.valueProperty().get().equals(now));

        // 6 check if onChangeListener was called (this was already working as expected before the patch )
        if (typo)
...
//                    commitValue();//super.commitValueOnFocusLost();
//                }
            };
